...
def write_dssp(ss, query_path, outpath):
    '''
    ...
    ID = os.path.basename(query_path)[:-8]
    dssp_header = '>'+ID 
...
if __name__ == "__main__":
    ...
    # List of queries to loop on
    query_list = listdir_nohidden(args.queryprofiles)
    # Directory obtaining ss predictions in fasta like .dssp
    pred_dir = args.output
    #---------------------------------------------------------------------------------------------------------------------------
    # Initiate loop
    #---------------------------------------------------------------------------------------------------------------------------
    for profile in query_list:
        pred_ss = predict_ss(log_H, log_E, log_C, log_marg_p_R, log_marg_p_SS, profile, win_size)
        write_dssp(pred_ss, profile, pred_dir)
